...
| `dwn-rs-stores` | Store implementations, plus query glue. | Adapt as reference only. Enbox native storage should target SQLite first, SurrealDB is no longer supported. | Traits move to `enbox-dwn-store`; SQLite implementation goes to `enbox-dwn-sqlite`; SurrealDB has been removed from the active workspace. |
