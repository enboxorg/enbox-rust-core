...
#[tokio::test]
async fn duplicate_wakes_do_not_duplicate_delivery() {
    ...
    for index in 0..3 {
        harness
            .commit_delete(TENANT, &format!("m{index}"), "2025-01-01T00:00:00.000000Z")
            .await;
        // Spurious repeats of a hint the store already published.
        harness.publish_wake(TENANT, 1);
        harness.publish_wake(TENANT, 99);
    }
...
#[tokio::test]
async fn a_wake_ahead_of_the_committed_feed_delivers_nothing() {
    let harness = live_harness().build().await;
    ...
    harness.publish_wake(TENANT, 42);
