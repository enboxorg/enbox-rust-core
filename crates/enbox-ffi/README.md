...
5. Poll UI state: `sync_status({"tenant":"…","remote":"https://dwn.example/"})`

`sync_once` returns a JSON [`SyncOnceResult`](../../crates/dwn-rs-core/src/sync.rs) mirroring the native engine. HTTP remotes use `@enbox/dwn-server` JSON-RPC via [`HttpSyncEndpoint`](../dwn-rs-core/src/sync_endpoint.rs).
